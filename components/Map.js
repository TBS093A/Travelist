...
import Interface from './MapInterface';
import CurrentArticle from './CurrentArticle';

class MapSvgGenerator extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      regID: '',
      regName: 'Wybierz Region',
      regArts: [],
      activeArt: 0,
      activeArtTab: [],
      comments: [],
    ...
    this.giveArtID = this.giveArtID.bind(this);
  ...
  handleClick( e, name ) {
    ...
    .then(function (response) {
        //handle success
        self.setState({ regArts: response.data });
        //console.log(self.state.regArts);
      })
  ...
  giveArtID( e, artID ) {         //dla Interface
    var self = this;
    self.setState({ activeArt: artID });
    for(var i = 0; i < self.state.regArts.length; i++)
      if(artID == self.state.regArts[i].articleID)
        self.setState({ activeArtTab: self.state.regArts[i] });
    self.giveComments( artID );
  }

  giveComments( artID ) {    //dla CurrentArticle
    var self = this;
    let formData = new FormData();
    formData.append('artID', artID);
    axios({
        method: 'post',
        url: 'api/giveComments.php',
        data: formData,
        withCredentials: true,
        config: { headers: {'Content-Type': 'multipart/form-data' }}
    })
  ...
  render(){
    ...
    return(
      <div>
        <div id={ this.state.width >= 1800 ? 'mapImg' : 'mapImgSM' }>
          <div id="divMapBackground">
            <div id={ this.state.width >= 1800 ? 'generalDivMap' : 'generalDivMapSM' }>
              ...
              <div id={ this.state.width >= 1800 ? 'divInterfaceMap' : 'divInterfaceMapSM' }>
                <Interface
                  handleClick={ this.handleClick }
                  giveArtID={ this.giveArtID }
                  regID={ this.state.regID }
                  regName={ this.state.regName }
                  regArts={ this.state.regArts }
        ...
        <div>
          <CurrentArticle
            giveComments={ this.giveComments }
            comments={ this.state.comments }
            activeArt={ this.state.activeArt }
            activeArtTab={ this.state.activeArtTab }
            regName={ this.state.regName }/>
        </div>
      </div>
    )
...
export default MapSvgGenerator
